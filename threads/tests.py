from django.test import TestCase
from django.shortcuts import render_to_response
from .models import Thread
from .models import Subject


class SubjectPageTest(TestCase):
    def test_check_content_is_correct(self):
        subject_page = self.client.get('/forum/')
        self.assertTemplateUsed(subject_page, "forum/forum.html")
        subject_page_template_output = render_to_response("forum/forum.html",
                                                          {'subjects':
                                                               Subject.objects.all()}).content
        self.assertEqual(subject_page.content, subject_page_template_output)

# My own view test (not working
class ThreadPageTest(TestCase):
    def test_check_content_is_correct(self):
        thread_page = self.client.get('/forum/')
        self.assertTemplateUsed(thread_page, "forum/threads.html")
        thread_page_template_output = render_to_response("forum/thread.html",
                                                                  {'thread':
                                                                       Thread.objects.all()}).content
        self.assertEqual(thread_page.content, thread_page_template_output)

                # My own url test